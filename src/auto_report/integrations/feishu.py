...
def build_feishu_card_payload(receive_id: str, card: dict[str, object]) -> dict[str, object]:
    return {
        "receive_id": receive_id,
...
def send_feishu_card_message(
    ...
    response = requests.post(
        f"{base_url}/open-apis/im/v1/messages?receive_id_type=chat_id",
        headers={"Authorization": f"Bearer {access_token}"},
        json=build_feishu_card_payload(receive_id, card),
        timeout=timeout,
    )
    response.raise_for_status()
    send_data = response.json()
...
def send_feishu_notification_with_fallback(
    app_id: str,
    app_secret: str,
    receive_id: str,
    text: str,
