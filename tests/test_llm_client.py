...
class TestProviderConfigResolution:
    ...
    def test_deepseek_stage_defaults_use_pro_for_key_stages(self):
        with patch.dict(os.environ, {}, clear=True):
            assert _resolve_provider_config(stage="analysis")["model"] == "deepseek-flash"
            assert _resolve_provider_config(stage="summary")["model"] == "deepseek-flash"
            assert _resolve_provider_config(stage="forecast")["model"] == "deepseek-flash"
            assert _resolve_provider_config(stage="prefilter")["model"] == "deepseek-flash"
    ...
    def test_is_llm_enabled_when_only_stage_specific_provider_key_exists(self):
        with patch.dict(
            os.environ,
            {
                ...
                "ANALYSIS_AI_MODEL": "deepseek-flash",
