...
| 变量 | 默认值 | 说明 | 是否必须 |
|------|--------|------|-----------|
| `DEEPSEEK_API_KEY` | *(空)* | DeepSeek 的 API key；负责 AI 分析、总结、预测。 | ✅（当 `AI_PROVIDER=deepseek`） |
| `OPENAI_API_KEY` | *(空)* | `AI_PROVIDER=openai` 时使用的 API key。 | ✅（当 `AI_PROVIDER=openai`） |
| `AI_API_KEY` | *(空)* | 自定义 OpenAI 兼容服务时使用的通用 API key。 | ✅（当 `AI_PROVIDER` 为自定义 provider） |
| `AI_PROVIDER` | `deepseek` | 选择 `deepseek`、`openai`，或自定义 OpenAI 兼容 provider。 | ✅ |
| `AI_BASE_URL` | `https://api.deepseek.com` | 覆盖默认的服务端点，用于私有部署或代理。 | 可选 |
...
- `AI_PROVIDER`：选择 `deepseek`、`openai` 或自定义 provider，决定 `integrations/llm_client.call_llm` 走哪套适配器。
- `OPENAI_API_KEY` / `AI_API_KEY`：分别用于官方 OpenAI 与自定义 OpenAI 兼容服务。
...
3. 如果要接入其他 OpenAI 兼容服务，将 `AI_PROVIDER` 设为自定义名称，并配置 `AI_API_KEY`、`AI_BASE_URL`、`AI_MODEL`。
4. `AI_MAX_ANALYSIS_TOPICS` 为 6 时意味着 pipeline 只把优先级最高的 6 个主题送到 AI，避免耗时过长。调整此值需对分析时长和成本进行权衡。
5. 如果 `DEEPSEEK_API_KEY`（或其他 key）缺失，或 DeepSeek/OpenAI 多次调用失败，管线会降级为“规则摘要”模式：AI 阶段跳过，仍然写出结构化主题、要点和预测，并继续推送。
6. `AI_BASE_URL` 可指向私有部署或旁路代理，务必确认该主机可访问，且符合 TLS 证书标准，否则流程会落到第五步的规则摘要模式。

`settings.py` 会把这些变量封装进 `Settings` dataclass，`app.py` 负责记录运行阶段耗时与 `run-status.json`，`integrations/llm_client.py` 负责低耦合的 API 调用；修改配置后记得同步更新这些文件。

## 校验与调试建议

- `.env.example` 给出了所有可配置字段。任何新增字段都要在 `README`/`USER_GUIDE`/`docs/push-channels-guide.md` 中提及，确保有人知道它们的作用。
- 运行 `python -m auto_report.cli run-once` 时，`data/state/run-status.json` 中的 `stage_status`、`timings`、`error` 与 `delivery_results` 会显示 AI、渲染和送达是否成功。
- 使用 `python -m auto_report.cli analyze-only` 可以单独验证 AI 阶段，但前提是已经存在 `data/state/intermediate.json`。
