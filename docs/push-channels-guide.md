...
- 默认发布轨为 `auto`；如需补发人工复核版，可在本地命令使用 `--publication-mode reviewed --reviewer <name> --review-note <text>`，或在 GitHub 手动触发时把 workflow input `publication_mode=reviewed`，并可选填写 `reviewer / review_note`。
- 三条通道的正文都固定携带两个阅读入口：
  - `公开阅读： https://wsysgz.github.io/VS_AI/`
  - `GitHub 原文： https://github.com/wsysgz/VS_AI/blob/main/data/reports/latest-summary-*.md`
- 公开站链接始终使用站点首页，不切 archive URL；reviewed 轨由站点首页优先展示 reviewed 完成公开面排序。
...
- 消息内容：PushPlus 使用固定 `short + risk-alert` 模板，发送一句判断 + 2~3 条主线 + 风险提醒 + 双阅读入口；最终以 `txt` 文本送达。若当前是 reviewed 轨，标题会带人工复核标记，GitHub 原文链接会自动切到 reviewed 文件，详见 `integrations/pushplus.py` 与 `outputs/renderers.py`。
...
- Telegram 使用固定 `long + reviewed-long` 模板，系统会发送执行摘要、关键主线、重点主题、短期观察、风险提醒与双阅读入口，并自动拆成 `<=4096` 字符段落顺序发送；reviewed 轨会在标题中显式标记人工复核版，并追加复核信息。
...
- Feishu 使用固定 `medium + reviewed-note` 模板，当前发送一句判断、执行摘要、关键主线、行动建议、风险提醒与双阅读入口；机器人通过 `tenant_access_token` + `im/v1/messages` API 发送纯文本正文，reviewed 轨同样会追加复核信息。
...
- PushPlus / Telegram / Feishu 都会附带公开站入口和 GitHub 原文链接；因此即使某个通道失败，成功送达的通道仍能同时提供公开阅读入口和精确回溯入口。
