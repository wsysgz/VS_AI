...
| `python -m auto_report.cli backfill --start YYYY-MM-DD --end YYYY-MM-DD` | 补报历史日期 | 需要手动指定日期范围 |

## 验证输出

- 查看 `data/reports/latest-summary.md` 与 `data/reports/latest-report.html` 确认内容；`data/state/run-status.json` 记录 `generated_files`、`timings` 和 `pushes`。
...
- 某些失败源于配置问题（如缺少 `DEEPSEEK_API_KEY` 或 Telegram token），请参考 `docs/TECHNICAL_GUIDE.md` 和 `docs/push-channels-guide.md`，修复 `.env` 或 Secrets。
