...
- PushPlus/WeChat 接收短文本摘要；Feishu 接收中长度结构化摘要；Telegram 接收完整长文报告（自动按 4096 字符分段）；三端正文都会同时附带公开站入口 `https://wsysgz.github.io/VS_AI/` 和当前 GitHub 原文链接。
...
- workflow 本地快验已固定为 `daily / recovery / full` 三档；上传前统一执行 `pwsh ./scripts/check-workflows.ps1 -Profile full`。
...
- 发布面现已支持 `auto / reviewed` 双轨：定时主链路默认产出 `auto`，手工 rerun / backfill 可切到 `reviewed`，并可选写入 `reviewer / review_note`；系统会同时保留 `latest-summary-auto.*` 与 `latest-summary-reviewed.*`，公开 Pages 在同一天两条轨同时存在时优先展示 reviewed。
- PromptOps 已提供正式仓库内资产：`config/ai_reading/registry.json` 管理 prompt 清单，`config/prompt_eval/baseline-v1.json` 提供离线 benchmark dataset；`evaluate-prompts --dataset <path>` 会把结果写入 `out/evals/`，并在私有 ops dashboard 中展示最近 prompt-eval 趋势与回归降分。
