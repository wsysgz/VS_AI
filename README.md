...
- 最新远端分支头：`aa2fff78 chore: update pages site [skip ci]`
- 本地最新批次：补入 `amd-newsroom`、`ernie-blog`，补齐启用源 comparison metadata，并夯实分析链的证据归因、enrichment 触发和国内外对比语义
...
- `ernie-blog` 当前决策是保留官方 RSS；`amd-newsroom` 当前决策是保留官方 newsroom listing；`nxp-edge-ai` 继续维持 repo fallback，不将不稳定官方 blog 直接接入主链
- 分析链当前语义：`support_evidence` 只保留 `direct_support`，`related_context` 仅作背景；`comparison_brief.track_snapshots` 表示同赛道代表信号，`head_to_head` 只表示语义上可直接对照的中外配对
